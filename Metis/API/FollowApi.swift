...
class FollowApi {
    ...
    func followAction(withUser id: String) {
        ...
        REF_FOLLOWERS.child(id).child(Api.Userr.CURRENT_USER!.uid).setValue(true)
        REF_FOLLOWING.child(Api.Userr.CURRENT_USER!.uid).child(id).setValue(true)
    }
    ...
    func unFollowAction(withUser id: String) {
        ...
        REF_FOLLOWERS.child(id).child(Api.Userr.CURRENT_USER!.uid).setValue(NSNull())
        REF_FOLLOWING.child(Api.Userr.CURRENT_USER!.uid).child(id).setValue(NSNull())
    }
    
    func isFollowing(userId: String, completed: @escaping (Bool) -> Void) {
        REF_FOLLOWERS.child(userId).child(Api.Userr.CURRENT_USER!.uid).observeSingleEvent(of: .value, with: {
            snapshot in
